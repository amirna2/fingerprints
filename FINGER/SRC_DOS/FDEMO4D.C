...
// =====
// Determine le contour de l'image Source vers une Image Cible
// =====
...
// =====
// Regroupement des pixels orient‚s … 45ø et … 135ø
// =====
void groupe(int huge *ImageSrc, int huge *ImageDest)
{
  int y,x,dep;

	for (y=0; y < Image.ligne; y++)    // Parcours de la matrice Image
	{
		for (x=0; x < Image.colonne; x++)
		{
			pix = y * Image.colonne + x;       
			if (ImageSrc[pix+Image.colonne+1]) // Si Pixel = centre d'une zone 3x3
			{
			...
			int lorg,int corg,int lend,int cend,UL ma[4][4] )
{
  ...
  char txt[80],t1[20],t2[20],t3[20],t4[20],t5[20];
  UL dj,p,nby,nbc,nbr,nbb,nbp;
  nbp=nby=nbc=nbb=nbr=0;
  ...
  for (y=lorg; y < lend; y=y+1)
  {
    for (x=corg; x <=cend-1; x=x+1)
    {
	 ...
	 switch ( ImageSrc[pix] )
	 {
	   case YELLOW    : nby++;break;
	   case LIGHTCYAN : nbc++;break;
	   case LIGHTRED  : nbr++;break;
	   case LIGHTBLUE : nbb++;
	 }
  ...
  if (nbp)
  {
    sprintf(t1,"%3.2f%%", 100.0*((float)nby/(float)nbp) );
    sprintf(t2,"%3.2f%%", 100.0*((float)nbr/(float)nbp) );
    sprintf(t3,"%3.2f%%", 100.0*((float)nbc/(float)nbp) );
    sprintf(t4,"%3.2f%%", 100.0*((float)nbb/(float)nbp) );
  }
  else
  {
    sprintf(t1,"%3.2f%%", 0);
  ...
  // Calcul de la densit‚ : Nb moyen de pixels voisins selon 4 directions
  if (nbpix)
    sprintf(t5,"DENSITY = %2.3f", ((float)nbpix/(float)nbp) );
  else
    sprintf(t5,"DENSITY = %2.3f", 0);

  setcolor(WHITE);
  outtextxy(0,370,"TOT.");
  setcolor(LIGHTGREEN);
  sprintf(txt,"PIXELS = %lu",nbp); outtextxy(40,370,txt);
  sprintf(txt,"MATRIX = %lu",nbpix); outtextxy(200,370,txt);
  outtextxy(340,370,t5);
  setcolor(WHITE);
  outtextxy(0,390,"FRQ.");
  setcolor(co0);   outtextxy(40,390,t1);
  setcolor(co135); outtextxy(120,390,t2);
  setcolor(co90);  outtextxy(200,390,t3);
  setcolor(co45);   outtextxy(280,390,t4);

  setcolor(co0);   outtextxy(40,410, "0ø");
  setcolor(co135); outtextxy(120,410,"135ø");
  setcolor(co90);  outtextxy(200,410,"90ø");
  setcolor(co45);  outtextxy(280,410,"45ø");

  setcolor(co0);   outtextxy(0,420,"0ø");
  setcolor(co135); outtextxy(0,430,"135ø");
  setcolor(co90);  outtextxy(0,440,"90ø");
  setcolor(co45);  outtextxy(0,450,"45ø");
  setcolor(WHITE);
  ...
  for (k=yellow;k<=blue;k++)
  {
    for (l=yellow;l<=blue;l++)
    {
     if (nbpix)
       ...
       outtextxy(40+80*k,420+10*l,txt);
    ...
    for (y = 0; y < Image.ligne; y=y+1)
    {
      for (x = 0; x < Image.colonne; x=x+1)
      {
				if (ImageSrc[p])
					...
					putpixel(x+xoffset,y+yoffset,BLACK);
	      p++;
      }
    }
  }
...
// Affiche un message sur le derniere ligne de l'ecran
...
void message2(int x,int y,char *txt)
{
  setcolor(LIGHTBLUE);
  outtextxy(x,y,txt);
	while (!getch());
...
void main( int argc, char *argv[])
{
	...
	if (argc != 2)
	{
	  ...
	  printf("Usage is : FDEMO <file.bmp>\n");
	  exit(1);
	}
	co0   = YELLOW;
	co45  = LIGHTBLUE;
...
// Affiche ImageOut
	ViewImage(ImageInit,0,10,10);
	window(9,9,col+10,lin+10);
	sprintf(msg,"Fichier : %s",argv[1]);
	setcolor(WHITE);
	outtextxy(9,lin+15,msg);
	message2(310,10,"[1]  IMAGE INITIALE...");
...
// Affiche ImageOut
	...
	ViewImage(ImageOut,0,10,10);
	window(9,9,col+10,lin+10);
	message2(310,30,"[2]  EROSION INITIALE...");
...
// Groupe pixel 45ø 135ø
	...
	ViewImage(ImageOut,0,10,10);
	window(9,9,col+10,lin+10);
	message2(310,50,"[3]  CONTOUR ET DIRECTIONS 45ø & 135ø...");
...
// Affiche le contour ‚rod‚ : Image2
	...
	ViewImage(Image2,0,10,10);
	window(9,9,col+10,lin+10);
	message2(310,70,"[4]  EROSION CONTOUR...");
...
// affiche r‚sultat de New_directions2
	...
	ViewImage(ImageOut,0,10,10);
	window(9,9,col+10,lin+10);
	message2(310,90,"[5]  DIRECTIONS 0ø & 90ø...");
...
// Affichage de Image2
	...
	ViewImage(Image2,0,10,10);
	window(9,9,col+10,lin+10);
	message2(310,110,"[6]  EROSION FINALE...");
...
// ZONE GLOBALE
  ...
  matrix_co4d(Image2,0, 0, lin, col, mat5);
	...
	message2(310,130,"[7]  MATRICE ZONE GLOBALE...");
	clear_zone(0,360,600,460);
// ZONE 1
  nbpix=0;
  window(10,10,10+col/2,10+lin/2);
  ...
  matrix_co4d(Image2, 0, 0, lin/2, col/2,mat);
	...
	message2(310,150,"[8]  MATRICE ZONE 1...");
  clear_zone(0,360,600,460);
// ZONE 2
  window(10+1+col/2, 10, 9+col,10+lin/2);
  nbpix=0;
  ...
  matrix_co4d(Image2, 0, 1+col/2, lin/2 ,col, mat2);
	...
	message2(310,170,"[9]  MATRICE ZONE 2...");
  clear_zone(0,360,600,460);
// ZONE 3
  window(10, 10+1+lin/2, 10+col/2, 9+lin);
  nbpix=0;
  ...
  matrix_co4d(Image2,1+lin/2, 0, lin, col/2, mat3);
	...
	message2(310,190,"[10]  MATRICE ZONE 3...");
	clear_zone(0,360,600,460);

// ZONE 4
  window(10+1+col/2, 10+1+lin/2, 9+col, 9+lin);
  nbpix=0;
  ...
  for (x=yellow;x<=blue;x++)
	...
	message2(310,210,"[11]  MATRICE ZONE 4...");
  clear_zone(0,360,600,460);
	fprintf(out,"%s\n",argv[1]);
	fclose(out);
	closegraph();
